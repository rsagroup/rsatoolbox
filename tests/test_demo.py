...
class TestDemos(unittest.TestCase):
    ...
    def test_example_dissimilarities(self):
        ...
        _ = rsr.calc_rdm(data, method="correlation", descriptor="conds")
    ...
    def test_dual_boot(self):
        ...
        matlab_data = io.matlab.loadmat(
            os.path.join(path, "..", "demos", "rdms_inferring", "modelRDMs_A2020.mat")
        )
        ...
        matlab_data = io.matlab.loadmat(
            os.path.join(
                path, "..", "demos", "rdms_inferring", "noisyModelRDMs_A2020.mat"
            )
        )
        ...
        for i_model in np.unique(model_names):
            models_flex.append(
                rsatoolbox.model.ModelSelect(
                    i_model, model_rdms.subset("brain_computational_model", i_model)
                )
            )
    ...
    def test_exercise_all(self):
        ...
        rsatoolbox.vis.show_rdm(conv1_rdms, rdm_descriptor="measurement_model")
        ...
        rdms_matlab = matlab_data["noisyModelRDMs"]
        repr_names = [
            repr_names_matlab[i][0][0] for i in range(repr_names_matlab.shape[0])
        ]
        ...
        models = []
        for i_model in np.unique(model_names):
            rdm_m = model_rdms.subset("brain_computational_model", i_model).subset(
        ...
        rsatoolbox.vis.plot_model_comparison(
            results_3_cv, error_bars=None, test_type=None
        )
    ...
    def test_temporal_rsa(self):
        ...
        dat = pickle.load(
            open(
                os.path.join(
                    path, "..", "demos", "TemporalSampleData", "meg_sample_data.pkl"
                ),
                "rb",
            )
        )
        ...
        print("shape of original measurements")
        print(data.measurements.shape)
        data_subset_time = data.subset_time("time", t_from=-0.1, t_to=0.5)
        print("\nafter subsetting")
        ...
        rdms_data = calc_rdm_movie(data, method="euclidean", descriptor="conds")
        print(rdms_data)
        rdms_data_binned = calc_rdm_movie(
            data, method="euclidean", descriptor="conds", bins=bins
        )
        ...
        plt.figure(figsize=(10, 10))
        rsatoolbox.vis.show_rdm(
            model_rdms, rdm_descriptor="model_names", pattern_descriptor="cond_names"
        )
        ...
        for i, r_ in enumerate(r):
            plt.plot(
                rdms_data_binned.rdm_descriptors["time"],
                r_.squeeze(),
                label=model_names[i],
            )
        plt.xlabel("time")
    ...
    def test_demo_rdm_scatterplot(self):
        ...
        condition_descriptors = {"type": (["A"] * 23) + (["B"] * 23) + (["C"] * 46)}

        matlab_data = loadmat(
            os.path.join(path, "..", "demos", "92imageData/92_brainRDMs.mat")
        )["RDMs"]
