...
def input_check_model(models, theta=None, fitter=None, N=1):
    ...
    if isinstance(models, Model):
        ...
        raise ValueError(
            "model should be an rsatoolbox.model.Model or a list of" + " such objects"
        )
    if N > 1:
    ...
    if theta is not None:
        ...
        assert len(models) == len(theta), (
            "there should equally many" + " models as parameters"
        )
    else:
...
def all_tests(
    ...
    if test_type == "t-test":
        ...
        p_noise = t_test_nc(
            evaluations, noise_ceil_var[:, 0], np.nanmean(noise_ceil[0]), dof
        )
...
def bootstrap_pair_tests(evaluations):
    ...
    proportions = (len(evaluations) - 1) / len(evaluations) * proportions + 1 / len(
        evaluations
    )
...
def extract_variances(variance, nc_included=True):
    ...
    if variance.ndim == 1:
        ...
        if nc_included:
            ...
            nc_variances = np.expand_dims(model_variances, -1) + np.expand_dims(
                variance[-2:], 0
            )
        ...
        if nc_included:
            ...
            nc_variances = np.array([model_variances, model_variances]).transpose(
                1, 2, 0
            )
