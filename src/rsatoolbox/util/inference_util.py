...
from typing import TYPE_CHECKING, Optional, List, Callable, Tuple
import warnings
import numpy as np
...
def input_check_model(
    models: list[Model],
    theta: Optional[List[NDArray]] = None,
    fitter: Optional[Callable | List[Callable]] = None,
    N: int = 1,
) -> Tuple[list[Model], NDArray, List[NDArray], List[Callable]]:
    ...
    if isinstance(models, Model):
        ...
        raise ValueError("model should be an rsatoolbox.model.Model or a list of" + " such objects")
    if N > 1:
        evaluations = np.zeros((N, len(models)))
    else:
        evaluations = np.zeros(len(models))
    if theta is not None:
        assert isinstance(theta, Iterable), (
            "If a list of models is" + " passed theta must be a list of parameters"
        )
        assert len(models) == len(theta), "there should equally many" + " models as parameters"
    else:
        theta = [None] * len(models)
    if fitter is None:
        fitter = [None] * len(models)
    elif isinstance(fitter, Iterable):
        assert len(fitter) == len(models), (
            "if fitters are passed " + "there should be as many as models"
        )
    else:
...
def pool_rdm(rdms, method: str = "cosine"):
    ...
    if method == "euclid":
        ...
        rdm_vec = rdm_vec / np.sqrt(np.nanmean(rdm_vec**2, axis=1, keepdims=True))
        rdm_vec = _nan_mean(rdm_vec)
    elif method == "corr":
...
def all_tests(
    ...
    if test_type == "t-test":
        ...
        p_noise = t_test_nc(evaluations, noise_ceil_var[:, 0], np.nanmean(noise_ceil[0]), dof)
...
def zero_tests(evaluations, test_type="t-test", model_var=None, dof=1):
...
def nc_tests(evaluations, noise_ceil, test_type="t-test", noise_ceil_var=None, dof=1):
...
def ranksum_pair_test(evaluations):
    ...
    assert evaluations.ndim == 3, "provided evaluations array has wrong dimensionality"
    ...
    for i_model in range(n_model - 1):
        for j_model in range(i_model + 1, n_model):
            pvalues[i_model, j_model] = wilcoxon(evaluations[i_model], evaluations[j_model]).pvalue
...
def ranksum_value_test(evaluations, comp_value=0):
    ...
    for i_model in range(n_model):
        pvalues[i_model] = wilcoxon(evaluations[i_model] - comp_value).pvalue
    return pvalues
...
def bootstrap_pair_tests(evaluations):
    ...
    for i_model in range(evaluations.shape[1] - 1):
        for j_model in range(i_model + 1, evaluations.shape[1]):
            ...
            ) / (evaluations.shape[0] - np.sum(evaluations[:, i_model] == evaluations[:, j_model]))
            proportions[j_model, i_model] = proportions[i_model, j_model]
    proportions = np.minimum(proportions, 1 - proportions) * 2
    proportions = (len(evaluations) - 1) / len(evaluations) * proportions + 1 / len(evaluations)
...
def t_test_nc(evaluations, variances, noise_ceil, dof=1):
    ...
    for i, eval_i in enumerate(evaluations):
        t = (eval_i - noise_ceil) / np.sqrt(np.maximum(variances[i], np.finfo(float).eps))
        p[i] = 2 * (1 - stats.t.cdf(np.abs(t), dof))
    return p


def extract_variances(
    variance, nc_included: bool = True, n_rdm: Optional[int] = None, n_pattern: Optional[int] = None
):
    ...
    if variance.ndim == 1:
        ...
        if nc_included:
            ...
            nc_variances = np.expand_dims(model_variances, -1) + np.expand_dims(variance[-2:], 0)
        ...
        if nc_included:
            ...
            nc_variances = (
                np.expand_dims(model_variances, -1)
                - 2 * variance[:-2, -2:]
        ...
        if nc_included:
            ...
            nc_variances = (
                np.expand_dims(model_variances, -1)
                - 2 * variance[:, :-2, -2:]
                + np.expand_dims(np.einsum("ijj->ij", variance[:, -2:, -2:]), 1)
            )
            # np.diag(C@variances@C.T)
            diff_variances = np.einsum("ij,kjl,il->ki", C, variance[:, :-2, :-2], C)
        else:
            C = pairwise_contrast(np.arange(variance.shape[1]))
            model_variances = np.einsum("ijj->ij", variance)
            nc_variances = np.array([model_variances, model_variances]).transpose(1, 2, 0)
...
def _correct_1d(variance: NDArray, n_pattern: Optional[int] = None, n_rdm: Optional[int] = None):
...
def get_errorbars(model_var, evaluations, dof, error_bars="sem", test_type="t-test"):
    ...
    if error_bars.lower() == "sem":
        ...
        if test_type == "bootstrap":
            ...
            framed_evals = np.concatenate(
                (np.tile(np.array((-np.inf, np.inf)).reshape(2, 1), (1, n_models)), evaluations),
                axis=0,
            )
            ...
            errorbar_high = np.quantile(framed_evals, 1 - prop_cut, axis=0) - perf
        else:
            std_eval = np.sqrt(np.maximum(model_var, 0))
            errorbar_low = std_eval * tdist.ppf(prop_cut, dof)
            errorbar_high = std_eval * tdist.ppf(prop_cut, dof)
    else:
        raise ValueError(
            "computing errorbars: Argument "
...
def _dual_bootstrap(variances, n_rdm=None, n_pattern=None):
    ...
    if n_rdm is None or n_pattern is None:
        variance = 2 * (variances[1] + variances[2]) - variances[0]
        variance = np.maximum(np.maximum(variance, variances[1]), variances[2])
        variance = np.minimum(variance, variances[0])
    else:
        variance = (
        ...
        variance = np.maximum(
            np.maximum(variance, (n_rdm / (n_rdm - 1)) * variances[1]),
            (n_pattern / (n_pattern - 1)) * variances[2],
        )
        variance = np.minimum(variance, variances[0])
    return variance
