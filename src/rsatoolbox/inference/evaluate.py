...
def eval_dual_bootstrap(
    ...
    N: int = 1000,
    n_cv: int = 2,
    pattern_descriptor: str = "index",
    rdm_descriptor: str = "index",
    ...
    Args:
        ...
        verbose(int): verbosity level
            < 0: deactivate progress bar
            0[default]: show progress bar
    ...
    if k_pattern is None:
        n_pattern = len(np.unique(data.pattern_descriptors[pattern_descriptor]))
    ...
    for i_sample in tqdm.trange(N, disable=(verbose < 0)):
        sample, rdm_idx, pattern_idx = bootstrap_sample(
            data, rdm_descriptor=rdm_descriptor, pattern_descriptor=pattern_descriptor
        )
        sample_rdm = data.subsample(rdm_descriptor, rdm_idx)
        sample_pattern = data.subsample_pattern(pattern_descriptor, pattern_idx)
        if len(np.unique(rdm_idx)) >= k_rdm and len(np.unique(pattern_idx)) >= 3 * k_pattern:
    ...
    if use_correction and n_cv > 1:
        ...
        noise_ceil_nonan = np.mean(noise_ceil[:, eval_ok], -2).transpose([1, 0, 2])
        ...
        var_mean = np.einsum("ijk,ilk->kjl", matrix, matrix) / (matrix.shape[0] - 1)
        matrix_1 = np.concatenate([evals_1, noise_ceil_1], 1)
        matrix_1 -= np.mean(matrix_1, 0, keepdims=True)
        var_1 = (
            np.einsum("ijmk,ilmk->kjl", matrix_1, matrix_1)
            / (matrix_1.shape[0] - 1)
            / matrix_1.shape[2]
        )
        ...
        matrix = np.concatenate([evals_nonan, noise_ceil_nonan], 1)
        matrix -= np.mean(matrix, 0, keepdims=True)
        variances = np.einsum("ijk,ilk->kjl", matrix, matrix) / (matrix.shape[0] - 1)
    result = Result(
        models,
        evaluations,
        method=method,
...
def eval_fixed(
    ...
    Args:
        ...
        method(string): comparison method to use
        verbose(int): verbosity level (not used here, for consistency)
    ...
    noise_ceil = boot_noise_ceiling(data, method=method, rdm_descriptor="index")
    if data.n_rdm > 1:
        variances = np.cov(evaluations[0], ddof=0) / evaluations.shape[-1]
...
def eval_bootstrap(
    ...
    noise_min = []
    noise_max = []
    for i in tqdm.trange(N, disable=(verbose < 0)):
        sample, _, pattern_idx = bootstrap_sample(
            data, rdm_descriptor=rdm_descriptor, pattern_descriptor=pattern_descriptor
        )
        if len(np.unique(pattern_idx)) >= 3:
            for j, mod in enumerate(models):
                rdm_pred = mod.predict_rdm(theta=theta[j])
                rdm_pred = rdm_pred.subsample_pattern(pattern_descriptor, pattern_idx)
    ...
    if boot_noise_ceil:
        ...
        variances = np.cov(np.concatenate([evaluations[eval_ok, :].T, noise_ceil[:, eval_ok]]))
    else:
        eval_ok = np.isfinite(evaluations[:, 0])
        noise_ceil = np.array(
            boot_noise_ceiling(data, method=method, rdm_descriptor=rdm_descriptor)
        )
...
def eval_bootstrap_pattern(
    ...
    noise_max = []
    for i in tqdm.trange(N, disable=(verbose < 0)):
        sample, pattern_idx = bootstrap_sample_pattern(data, pattern_descriptor)
        if len(np.unique(pattern_idx)) >= 3:
            for j, mod in enumerate(models):
                rdm_pred = mod.predict_rdm(theta=theta[j])
                rdm_pred = rdm_pred.subsample_pattern(pattern_descriptor, pattern_idx)
...
def crossval(
    models: List[Model],
    rdms: RDMs,
    train_set: List[Tuple[RDMs, NDArray]],
    test_set: List[Tuple[RDMs, NDArray]],
    ceil_set: Optional[List[Tuple[RDMs, NDArray]]] = None,
    method: str = "cosine",
    fitter: Optional[List[Callable]] = None,
    pattern_descriptor: str = "index",
    calc_noise_ceil: bool = True,
) -> Result:
    ...
    """
    assert len(train_set) == len(test_set), "train_set and test_set must have the same length"
    if ceil_set is not None:
        assert len(ceil_set) == len(test_set), "ceil_set and test_set must have the same length"
    if isinstance(models, Model):
        models = [models]
    evaluations = []
    noise_ceil = []
    for i, train in enumerate(train_set):
        test = test_set[i]
        if train[0].n_rdm == 0 or test[0].n_rdm == 0 or train[0].n_cond <= 2 or test[0].n_cond <= 2:
            evals = np.empty(len(models)) * np.nan
        else:
            models, evals, _, fitter = input_check_model(models, None, fitter)
            ...
            if ceil_set is None and calc_noise_ceil:
                noise_ceil.append(
                    boot_noise_ceiling(
                        rdms.subsample_pattern(by=pattern_descriptor, value=test[1]), method=method
                    )
                )
    ...
    if ceil_set is not None and calc_noise_ceil:
        noise_ceil = cv_noise_ceiling(
            rdms, ceil_set, test_set, method=method, pattern_descriptor=pattern_descriptor
        )
    ...
    result = Result(
        models, evaluations, method=method, cv_method="crossvalidation", noise_ceiling=noise_ceil
    )
    return result
...
def bootstrap_crossval(
    ...
    method: str = "cosine",
    fitter: Optional[Callable] = None,
    k_pattern: Optional[int] = None,
    k_rdm: Optional[int] = None,
    N: int = 1000,
    n_cv: int = 2,
    pattern_descriptor: str = "index",
    rdm_descriptor: str = "index",
    ...
    for i_sample in tqdm.trange(N, disable=(verbose < 0)):
        if boot_type == "both":
            ...
            sample, rdm_idx = bootstrap_sample_rdm(data, rdm_descriptor=rdm_descriptor)
            pattern_idx = np.unique(data.pattern_descriptors[pattern_descriptor])
        else:
    ...
    if use_correction and n_cv > 1:
        ...
        var_1 = []
        for i in range(n_cv):
            var_1.append(np.cov(np.concatenate([evals_1[:, :, i].T, noise_ceil_1[:, :, i]])))
...
def eval_dual_bootstrap_random(
    ...
    if n_pattern is None:
        n_pattern_all = len(np.unique(data.pattern_descriptors[pattern_descriptor]))
    ...
    for i_sample in tqdm.trange(N, disable=(verbose < 0)):
        if boot_type == "both":
            ...
            raise ValueError("boot_type not understood")
        if len(np.unique(rdm_idx)) > n_rdm and len(np.unique(pattern_idx)) >= 3 + n_pattern:
            ...
            if n_rdm > 0 or n_pattern > 0:
                nc = cv_noise_ceiling(
                    sample, ceil_set, test_set, method=method, pattern_descriptor=pattern_descriptor
                )
            else:
                nc = boot_noise_ceiling(sample, method=method, rdm_descriptor=rdm_descriptor)
...
def _concat_sampling(sample1, sample2):
    ...
    and sample2
    """
    sample_out = [[i_samp1 for i_samp1 in sample1 if i_samp1 == i_samp2] for i_samp2 in sample2]
