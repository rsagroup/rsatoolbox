...
def eval_fixed(models, data, theta=None, method="cosine"):
    ...
    if data.n_rdm > 1:
        variances = np.cov(evaluations[0], ddof=1) / evaluations.shape[-1]
...
def eval_bootstrap(
    ...
    if boot_noise_ceil:
        ...
        variances = np.cov(
            np.concatenate([evaluations[eval_ok, :].T, noise_ceil[:, eval_ok]])
        )
    else:
        eval_ok = np.isfinite(evaluations[:, 0])
        noise_ceil = np.array(
...
def bootstrap_crossval(
    ...
    if boot_type == "both":
        ...
        dof = data.n_rdm - 1
    eval_ok = ~np.any(np.any(np.any(np.isnan(evaluations), axis=-1), axis=-1), axis=-1)
    if use_correction and n_cv > 1:
        # we essentially project from the two points for 1 repetition and
        # for n_cv repetitions to infinitely many cv repetitions
        ...
        for i in range(n_cv):
            var_1.append(
                np.cov(np.concatenate([evals_1[:, :, i].T, noise_ceil_1[:, :, i]]))
            )
...
def _concat_sampling(sample1, sample2):
    ...
    """
    sample_out = [
        [i_samp1 for i_samp1 in sample1 if i_samp1 == i_samp2] for i_samp2 in sample2
    ]
