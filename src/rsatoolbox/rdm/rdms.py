...
class RDMs:
    ...
    def __init__(
        ...
        self.dissimilarities, self.n_rdm, self.n_cond = batch_to_vectors(
            dissimilarities
        )
        ...
        if rdm_descriptors is None:
            ...
            check_descriptor_length_error(
                rdm_descriptors, "rdm_descriptors", self.n_rdm
            )
        ...
        if pattern_descriptors is None:
            ...
            check_descriptor_length_error(
                pattern_descriptors, "pattern_descriptors", self.n_cond
            )
    ...
    def append(self, rdm):
        ...
        self.rdm_descriptors = append_descriptor(
            self.rdm_descriptors, rdm.rdm_descriptors
        )
    ...
    for i_cat in range(n):
        for j_cat in range(i_cat + 1, n):
            if isinstance(category_vector[i_cat], Iterable):
                comparisons = [
                    np.array(category_vector[i_cat][idx])
