...
def calc_rdm(
    ...
    if isinstance(dataset, Iterable):
        ...
        if method == "euclidean":
            ...
            rdm = calc_rdm_crossnobis(dataset, descriptor, noise, cv_descriptor)
        elif method == "poisson":
...
def calc_rdm_movie(
    ...
    if isinstance(dataset, Iterable):
        ...
        for i_dat, ds_i in enumerate(dataset):
            if noise is None:
                rdms.append(calc_rdm_movie(ds_i, method=method, descriptor=descriptor))
            elif isinstance(noise, np.ndarray) and noise.ndim == 2:
                rdms.append(
                    calc_rdm_movie(
                        ds_i, method=method, descriptor=descriptor, noise=noise
                    )
                )
            elif isinstance(noise, Iterable):
                rdms.append(
                    calc_rdm_movie(
                        ds_i, method=method, descriptor=descriptor, noise=noise[i_dat]
                    )
                )
        rdm = concat(rdms)
    else:
...
def calc_rdm_mahalanobis(dataset, descriptor=None, noise=None):
    ...
    rdm = _extract_triu_(rdm) / measurements.shape[1]
    return _build_rdms(
        rdm, dataset, "squared mahalanobis", descriptor, desc, noise=noise
    )
...
def calc_rdm_crossnobis(dataset, descriptor, noise=None, cv_descriptor=None):
    ...
    if descriptor is None:
        raise ValueError(
            "descriptor must be a string! Crossvalidation"
    ...
    if (noise is None) or (isinstance(noise, np.ndarray) and noise.ndim == 2):
        for i_fold, fold in enumerate(cv_folds):
            ...
            data_train = datasetCopy.subset_obs(
                cv_descriptor, np.setdiff1d(cv_folds, fold)
            )
            ...
            for j_fold in range(i_fold + 1, len(cv_folds)):
                if i_fold != j_fold:
                    rdm = _calc_rdm_crossnobis_single(
                        measurements[i_fold],
                        measurements[j_fold],
                        np.linalg.inv((variances[i_fold] + variances[j_fold]) / 2),
                    )
                    rdms.append(rdm)
    rdms = np.array(rdms)
    rdm = np.einsum("ij->j", rdms) / rdms.shape[0]
    return _build_rdms(
        rdm, datasetCopy, "crossnobis", descriptor, noise=noise, cv=cv_descriptor
    )


def calc_rdm_poisson(dataset, descriptor=None, prior_lambda=1, prior_weight=0.1):
    """
    ...
    """
    measurements, desc = _parse_input(dataset, descriptor)
    measurements = (measurements + prior_lambda * prior_weight) / (1 + prior_weight)
...
def calc_rdm_poisson_cv(
    dataset, descriptor=None, prior_lambda=1, prior_weight=0.1, cv_descriptor=None
):
    """
    ...
    for i_fold in range(len(cv_folds)):
        ...
        data_train = dataset.subset_obs(cv_descriptor, np.setdiff1d(cv_folds, fold))
        measurements_train, _, _ = average_dataset_by(data_train, descriptor)
        measurements_test, _, _ = average_dataset_by(data_test, descriptor)
        measurements_train = (measurements_train + prior_lambda * prior_weight) / (
            1 + prior_weight
        )
        measurements_test = (measurements_test + prior_lambda * prior_weight) / (
            1 + prior_weight
        )
...
def _parse_input(
    dataset: DatasetBase, descriptor: Optional[str]
...
def _averaging_occurred(
    ds: DatasetBase, obs_desc_name: str | None, obs_desc_vals: NDArray | None
) -> bool:
...
def _compatible_desc(rdms):
    """checks whether a list of RDMs objects has the same pattern descriptors"""
    for r in rdms:
        if not (r.n_cond == rdms[0].n_cond):
            break
        for k, v in r.pattern_descriptors.items():
            if ~np.all(np.array(v) == np.array(rdms[0].pattern_descriptors[k])):
                break
    else:
        return True
    return False
