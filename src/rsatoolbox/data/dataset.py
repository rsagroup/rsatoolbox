...
class Dataset(DatasetBase):
    ...
    def split_channel(self, by):
        ...
        for i_v, v in enumerate(unique_values):
            ...
            channel_descriptors = subset_descriptor(self.channel_descriptors, selection)
            dataset = Dataset(
                measurements=measurements,
                descriptors=descriptors,
                obs_descriptors=obs_descriptors,
    ...
    def odd_even_split(self, obs_desc):
        ...
        """
        assert (
            obs_desc in self.obs_descriptors.keys()
...
class TemporalDataset(Dataset):
    ...
    def __init__(
        ...
        if check_dims:
            check_descriptor_length_error(
                obs_descriptors, "obs_descriptors", self.n_obs
            )
            check_descriptor_length_error(
                channel_descriptors, "channel_descriptors", self.n_channel
            )
            check_descriptor_length_error(
                time_descriptors, "time_descriptors", self.n_time
            )
    ...
    def __str__(self):
        ...
        return (
            f"rsatoolbox.data.{self.__class__.__name__}\n"
            f"measurements = \n{measurements}\n...\n\n"
            f"descriptors: \n{string_desc}\n\n"
            f"obs_descriptors: \n{string_obs_desc}\n\n"
    ...
    def split_time(self, by):
        ...
        for v in time:
            selection = [
                i for i, val in enumerate(self.time_descriptors[by]) if val == v
            ]
            measurements = self.measurements[:, :, selection]
            descriptors = self.descriptors
            obs_descriptors = self.obs_descriptors
    ...
    def bin_time(self, by, bins):
        ...
        time_descriptors["bins"] = [
            np.array2string(x, precision=2, separator=",") for x in bins
        ]
    ...
    def convert_to_dataset(self, by):
        ...
        for v in time:
            ...
            measurements = np.concatenate(
                (measurements, self.measurements[:, :, selection].squeeze()), axis=0
            )

            for key in self.obs_descriptors:
                obs_descriptors[key] = np.concatenate(
                    (obs_descriptors[key], self.obs_descriptors[key].copy()), axis=0
                )
            ...
            for key in self.time_descriptors:
                obs_descriptors[key] = np.concatenate(
                    (
                        obs_descriptors[key],
                        np.repeat(
                            [self.time_descriptors[key][s] for s in selection],
                            self.n_obs,
                        ),
                    ),
                    axis=0,
...
def merge_subsets(dataset_list):
    ...
    for ds in dataset_list[1:]:
        ...
        obs_descriptors = append_obs_descriptors(
            obs_descriptors, ds.obs_descriptors.copy()
        )
