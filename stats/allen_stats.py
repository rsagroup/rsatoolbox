...
# -*- coding: utf-8 -*-
"""
Functions to check the statistical integrity of the toolbox for allen data
"""
...
def run_allen(idx, allen_path=None, start_idx=0):
    ...
    if allen_path is None:
        allen_path = '~/pyrsa/demo/allendata/natural_scenes/'
    ...
    if variation[i_var][:4] == 'None':
        ...
        sim_allen(variation=variation[i_var],
                   layer=layer[i_layer],
                   n_repeat=n_repeat[i_repeat],
                   n_subj=n_subj[i_sub],
                   sd=sd[i_sd], boot_type=boot[i_var],
...
def sim_allen(layer=2, sd=0.05, n_stim_all=320,
    ...
    """ simulates representations based on allen data
    ...
    allen_path = pathlib.Path(allen_path).expanduser()
    ...
    for i in tqdm.trange(start_idx, n_sim):
        ...
        if os.path.isfile(fname_base + 'stim%04d.txt' % i):
            ...
            stim_paths = []
            folders = os.listdir(allen_path)
            for i_stim in range(n_stim_all):
                i_folder = np.random.randint(len(folders))
                images = os.listdir(os.path.join(allen_path,
                                                 folders[i_folder]))
        ...
        if not os.path.isfile(fname_base + 'stim%04d.txt' % i):
            with open(fname_base + 'stim%04d.txt' % i, 'w') as f:
                for item in stim_paths:
                    f.write("%s\n" % item)
        stim_list = get_stimuli_ecoset(allen_path, stim_paths[:n_stim])
