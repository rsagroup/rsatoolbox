...
def get_residuals_cross(designs, timecourses, betas, resolution=2, hrf=None):
    ...
    for iCross in range(len(designs)):
        ...
        residuals[iCross] = get_residuals(
            designs[iCross], timecourses[iCross], beta, resolution=resolution, hrf=hrf
        )
    return residuals


def check_compare_to_zero(
    model,
    n_voxel=200,
    n_subj=10,
    n_sim=100,
...
def check_compare_models_rand(
    ...
    for i_sim in tqdm.trange(n_sim, position=0):
        ...
        for i_subj in range(n_subj):
            ...
            data.append(dat)
        rdms = rsatoolbox.rdm.calc_rdm(data)
        model1 = rsatoolbox.model.ModelFixed(
...
def check_noise_ceiling(
    ...
    for i_sim in tqdm.trange(n_sim, position=0):
        ...
        results = run_inference(
            model, rdms, method, bootstrap, boot_noise_ceil=boot_noise_ceil
        )
        idx_valid = ~np.isnan(results.evaluations[:, 0])
        if test_type == "perc":
            if boot_noise_ceil:
                p_upper[i_sim] = np.sum(
                    results.evaluations[idx_valid, 0]
...
def save_noise_ceiling(
    ...
    fname = (
        folder
        + os.path.sep
        + "p_%s_%s_%s_%s_%d_%d_%d_%.2f_%03d.npy"
        % (
            method,
            bootstrap,
            test_type,
...
def sim_ecoset(
    ...
    for i in tqdm.trange(start_idx, n_sim):
            ...
            for i_stim in range(n_stim):
                U_complete.append(
                    dnn.get_complete_representation(
                        model=model, layer=layer, stimulus=stim_list[i_stim]
                    )
                )
        ...
        if i == start_idx or variation in ["stim", "both"]:
            models = get_models(
                model_type, stim_list, n_layer=n_layer, smoothing=smoothing
            )
        # calculate RDMs
        data = []
        desc = {
            "stim": np.tile(np.arange(n_stim), n_repeat),
...
def run_flex(idx, start_idx, simulation_folder="sim_flex", ecoset_path="~/ecoset/val/"):
    ...
    if not os.path.isfile(os.path.join(fname_base, res_name, fname)):
        sim_ecoset(
            layer=layer,
            sd=sd,
            n_stim_all=80,
...
def summarize_eco(simulation_folder="sim_eco"):
    ...
    variances = []
    pairs = []
    for layer in [i for i in os.listdir(simulation_folder) if i[:5] == "layer"]:
        i_layer = int(layer[-2:])
        for pars in os.listdir(os.path.join(simulation_folder, layer)):
            n_voxel, n_subj, n_rep, sd, variation = parse_pars(pars)
            for fmri in os.listdir(os.path.join(simulation_folder, layer, pars)):
                (
                    duration,
                    pause,
                    endzeros,
        ...
        for i_res in results.glob("res*.hdf5"):
            ...
            try:
                ...
                if res.evaluations.ndim == 2:
                    ...
                    no_nan_idx = ~np.any(
                        np.any(np.any(np.isnan(res.evaluations), axis=-1), axis=-1),
                        axis=-1,
                    )
                if np.any(no_nan_idx):
                    for i in range(12):
...
def summarize_boot_cv(simulation_folder="boot_cv"):
    ...
    for i, n_cv in enumerate(n_cvs):
        n_boot = n_boots[i]
        for results in pathlib.Path(
            os.path.join(simulation_folder, f"cv_{n_cv}_{n_boot}/")
        ).glob("res*"):
            sys.stdout.write(str(results) + "\n")
            ...
            means[i, :, i_sim, i_rep] = np.mean(
                np.mean(np.mean(res.evaluations, 0), 1), 1
            )
            ...
            eval_ok = ~np.any(
                np.any(np.any(np.isnan(res.evaluations), axis=-1), axis=-1), axis=-1
            )
        ...
        if os.path.isdir(folder_path):
            ...
            for par in pars:
                ...
                for fmri in fmris:
                    ...
                    break_results = False
                    for i in range(100):
                        if not os.path.isfile(os.path.join(path, "stim%04d.txt" % i)):
                            print(path)
                            print("stimuli incomplete")
                            break
                        if not os.path.isfile(
                            os.path.join(path, "indices_space%04d.npy" % i)
                        ):
                            print(path)
                        ...
                        for i_res in glob.glob(os.path.join(path, "results*")):
                            if not os.path.isfile(
                                os.path.join(i_res, "res%04d.hdf5" % i)
                            ):
                                print(i_res)
    ...
    for idx in indices:
        ...
        if os.path.isdir(fname_base):
            ...
            if os.path.isdir(os.path.join(fname_base, res_name)):
                start_idx = 0
                for i_res in pathlib.Path(os.path.join(fname_base, res_name)).glob(
                    "*res*.hdf5"
                ):
                    i = int(str(i_res)[-9:-5])
                    if i >= start_idx:
...
def boot_cv_sim(
    ...
    # get models
    models = get_models(model_type, stim_list, n_layer=12, smoothing=smoothing)
    # get true U RDMs
    # dat_true = []
    # for i_subj in range(U.shape[0]):
    #     dat_true.append(rsatoolbox.data.Dataset(Utrue[i_subj]))
