...
def map_model_comparison(result, rdms_data=None, RDM_dist_measure = 'corr',
    ...
    while len(evaluations.shape) > 2:
        evaluations = np.nanmean(evaluations, axis=-1)  # average across trailing dimensions
    ...
    if result.method == 'corr':
        inference_descr += ('Pearson correlation distance (proportional to squared Euclidean distance after RDM centering and divisive normalization). ')
    elif result.method == 'cosine':
        inference_descr += ('cosine distance (proportional to squared Euclidean distance after RDM divisive normalizaton). ')
    else:
        raise Exception('pyrsa.vis.map_model_comparison: result.method ' + result.method + ' not yet handled.')
    ...
    if RDM_dist_measure.lower() == 'corr':
        ...
        inference_descr += ('Euclidean distance (proportional to the square root of correlation or cosine distance if RDMs were appropriately normalized).')
        ...
        if n_col == n_models:  # one color passed for each model...
            ...
            for c in range(n_chan):
                cols2[:, c] = np.interp(np.array(range(n_models)),
                                        np.array(range(n_col))/(n_col-1)*(n_models-1),
                                        colors[:, c])
        colors = cols2
    ...
    if rdms_data is None:
        print('No data RDMs passed. Omitting noise correction. Data-model RDM distances will be positively biased.')
        bias_of_sq_data_model_dist = 0
    else:
        N = rdms_data.n_rdm
        if result.method in ['corr', 'cosine']:
            bias_of_sq_data_model_dist = 2*(1-noise_upper)/(N-1)
            print('SE-based bias_of_sq_data_model_dist: {:.4f}'.format(bias_of_sq_data_model_dist))
            if model_data_dist_bias_corr == 'bootstrap':
                ...
                SSQD = 0
                print('Bootstrapping to estimate the bias of the squared data-model RDM distances...', flush=True)
                sys.stdout.flush()
                ...
                print('\nBootstrap-based bias_of_sq_data_model_dist: {:.4f}'.format(bias_of_sq_data_model_dist))
        else:
            raise Exception('pyrsa.vis.map_model_comparison: RDM comparison method must be ''corr'' or ''cosine'' for current implementation.')
    
    # DEBUG
    #bias_of_sq_data_model_dist = 0  # no correction: upper bound is at the center (0)
    #bias_of_sq_data_model_dist = 2*(noise_upper - noise_lower)  # collapse the noise ceiling: lower bound is at the center (0)
    
    if RDM_dist_measure.lower() in ['corr', 'cosine']:
        dist_pow = 1
    elif RDM_dist_measure.lower() == 'euclidean':
        dist_pow = 1/2
    data_model_dists = ( 2 * (noise_upper - perf) - bias_of_sq_data_model_dist )**dist_pow  
    noise_halo_rad   = ( 2 * (noise_upper - noise_lower) - bias_of_sq_data_model_dist )**dist_pow
    ...
    intermodelDists = ssd.squareform(ssd.pdist(modelRDMs, metric='euclidean'))**(2*dist_pow)
    ...
    print('\nPerforming MDS to map model RDMs around the data RDM, using ' + MDS_method + ' MDS...', flush=True)
    sys.stdout.flush()
    ...
    r, r_model_data, Spearman_r_model_data = show_Shepard_plot(locs2d, rdm_dists, models, colors)
    print('Pearson r(RDM dist, 2d-map dist): {:.4f}'.format(r))
    print('Pearson r(RDM dist, 2d-map dist) for model-data dists: {:.4f}'.format(r_model_data))
    print('Spearman r(RDM dist, 2d-map dist) for model-data dists: {:.4f}'.format(Spearman_r_model_data))
    ...
    plt.scatter([-rng, 0, rng, 0],[0, rng, 0, -rng], c='none')  # ensure axes shows the largest orbit completely
            ...
            if not significant[i, j]:
                ...
                if (angle_diff >  0 and abs(angle_diff) <= np.pi) or \
                   (angle_diff <= 0 and abs(angle_diff) >  np.pi):
                    ...
                    angles = np.linspace(angle_i, angle_i + min(abs(angle_diff), 2*np.pi-abs(angle_diff)), 360)
                    radii  = np.linspace(rad_i, rad_j, 360)
                else:
                    # clockwise from j to i is shorter
                    angles = np.linspace(angle_j, angle_j + min(abs(angle_diff), 2*np.pi-abs(angle_diff)), 360)
    ...
    # plot orbits, relatedness and distinctness tests, error bars, and model labels 
    for model_i in range(n_models):
        # orbits
        v = locs2d[model_i+1]
        rad = np.sqrt(v*v.T)
        ...
        if not model_significant[model_i]:
            ...
            x1, y1 = np.array((x0, y0)) / np.sqrt(np.sum( x0**2 + y0**2 )) * r_max * clearance_fac * 0.98
            plt.plot([x0, x1], [y0, y1], color=ns_col, linewidth=ns_lw)  # plot outer non-significance ray
        ...
        if not model_below_lower_bound[model_i]:
            ...
            plt.plot([0, x], [0, y], color=ns_col, linewidth=ns_lw)  # plot outer non-significance ray
        ...
        if label_orientation == 'tangential':
            angle_deg = (np.arctan2(vn[0, 1], vn[0, 0])/np.pi*180)%180-90  # tangential
            horAlign = 'center'
            verAlign = 'bottom' if ty>0 else 'top'
        elif label_orientation == 'radial':
            angle_deg = (np.arctan2(vn[0, 1], vn[0, 0])/np.pi*180-90)%180-90  # radial
            horAlign = 'right' if tx<=1e-5 else 'left'
            verAlign = 'center'
        plt.text(tx, ty, models[model_i].name, va=verAlign, ha=horAlign, rotation_mode='anchor', family='sans-serif', size=fs, rotation = angle_deg)
        #plt.plot(tx, ty, 'r.')
        
    # plot the data RDM with its noise halo    
    noise_halo = plt.Circle((0, 0), noise_halo_rad, color=noise_halo_col, zorder=10)
    ...
    plt.plot([-rng, -rng+scalebar_length], [-rng, -rng], color='k', linewidth=6)
    plt.plot([-rng+scalebar_length*qnts[0], -rng+scalebar_length*qnts[1]], [-rng, -rng], color=[0.5, 0.5, 0.5], linewidth=2)
    ...
    plt.text(-rng + 0.5*scalebar_length, -rng*1.02, descr, va='top', ha='center', family='sans-serif', size=fs_small)
...
def custom_MDS(rdm_dists, n_init=100, n_iter=500):
    ...
    for init_i in tqdm(range(n_init)):
        ...
        locs2d[0] = 0, 0  # place data RDM at the origin and the best model straight above it
        locs2d[best_model_i+1] = 0, rdm_dists[0, best_model_i+1]  # ...and best model straight above it
        
        # Initialize by locally optimal placing of each of the remaining models in random order
        rand_perm_other_model_is = np.random.permutation(other_model_is)
        for model_i in rand_perm_other_model_is:
            locs2d[model_i+1, :] = place_model(model_i, locs2d, rdm_dists, 1, two)
        ...
        #     #locs2d[model_i+1, :] = np.concatenate([np.sin(angle)[:,None], np.cos(angle)[:,None]], axis=1) * radius
        ...
        for iter_i in range(n_iter):
            ...
            rand_perm_other_model_is = np.random.permutation(other_model_is)
            for model_i in rand_perm_other_model_is:
                locs2d[model_i+1, :] = place_model(model_i, locs2d, rdm_dists, n_scales, two)
            if np.max(abs(locs2d_prev - locs2d)) < 1e-8:
                #print('MDS converged after {:.0f} iterations using {:.0f} scales.'.format(iter_i, n_scales))
                n_scales += 1
        ...
        if ssqd < ssqd_min:
            ...
            r_0fixed = np.sum(rdm_dists_vec * dists_2d)/(np.sum(rdm_dists_vec**2)+np.sum(dists_2d**2))
            locs2d_best = locs2d.copy()
            print(' SSQD: {:.4f}, corr_0fixed(RDM dist, map dist): {:.4f}'.format(ssqd, r_0fixed))
        if iter_i == n_iter-1:
...
def place_model(model_i, locs2d, rdm_dists, n_scales=3, two=2):
    ...
    for scale_i in range(n_scales):
        ...
        cand_locs = np.concatenate([np.sin(angles)[:,None], np.cos(angles)[:,None]], axis=1) * radius
        dists_2d = np.sqrt(np.sum((cand_locs.reshape(n_angles, 1, 2) - locs2d.reshape(1,n_models+1,2))**2, axis=2))
        ssqd = np.nansum(abs(dists_2d - rdm_dists[model_i+1,:].reshape(1, n_models+1))**two, axis=1)
...
#%% Perform multidimensional scaling (without compromising the model-data RDM distances)  
def weighted_MDS(rdm_dists, n_weightings=1, n_MDS_runs=100):
    ...
    for weighting_i in range(n_weightings):
        for mds_run_i in tqdm(range(n_MDS_runs),desc='{:.0f} of {:.0f}'.format(weighting_i+1, n_weightings)):
            # perform weighted MDS
            xy_try = pyrsa.vis.mds(pyrsa_rdm_dists, dim=2, weight=w)
            xy_try = np.matrix(xy_try.squeeze())
            r_try = np.corrcoef(rdm_dists_vec, ssd.pdist(xy_try, metric='euclidean'))[0, 1]
            Spearman_r_model_data_try = \
                sst.spearmanr(rdm_dists_vec[:n_models], \
            ...
            if r_try > r:
                ...
                if Spearman_r_model_data_try == 1.0:
                    ...
                    xy = xy_try
                    print('Improved map preserves model ranks: r(dist RDM, dists_2d) = {:.3f}'.format(r))
        w = w**0.9
    
    if r == 0:
        raise Exception('pyrsa.vis.map_model_comparison: Could not find map that preserves model performance ranks.')
...
def show_Shepard_plot(locs2d, rdm_dists, models, colors):
    ...
    r_model_data = np.corrcoef(rdm_dists_vec[:n_models], dists_2d_vec[:n_models])[0,1]
    Spearman_r_model_data = sst.spearmanr(rdm_dists_vec[:n_models], dists_2d_vec[:n_models]).correlation
    
    # Shepard plot
    plt.figure(figsize=(fig_width, fig_width), dpi=dpi)
    plt.plot((0, mx), (0, mx), color=[0.5, 0.5, 0.5, 0.5])
    plt.plot(rdm_dists_vec, dists_2d_vec, '.', color=[0, 0, 0, 0.8], mec=[1, 1, 1], ms=20, label='model-model distances')
    plt.plot(rdm_dists_vec[:n_models], dists_2d_vec[:n_models], '.', color=[0.8, 0, 0], mec=[1, 1, 1, 1], ms=20, label='model-data distances')
    plt.title('Shepard plot\n(Pearson r: {:.3f}, Pearson r for model-data dists: {:.1f})'.format(r, r_model_data), fontsize=fs_large)
    plt.xlabel('distance between RDMs', fontsize=fs)
    plt.ylabel('2d distance', fontsize=fs)
    plt.xticks(fontsize=fs_small)
    plt.yticks(fontsize=fs_small)
    #plt.axis('equal')  # does not make the axis square
    plt.axis([0, mx, 0, mx])
    ...
    dist_2d_max = np.max(dists_2d_vec)
    scalebar_length = round(approx_frac_of_max * dist_2d_max * 10)/10
    plt.plot([0,mx], [0, mx*qnts[0]], color=[0.5, 0.5, 0.5, 0.2], linewidth=1)
    plt.plot([0,mx], [0, mx*qnts[1]], color=[0.5, 0.5, 0.5, 0.2], linewidth=1)
    plt.plot([scalebar_length, scalebar_length], [0, scalebar_length], color=[0, 0, 0, 0.7], linewidth=6)
    plt.plot([scalebar_length, scalebar_length], [scalebar_length*qnts[0], scalebar_length*qnts[1]], color=[0.5, 0.5, 0.5], linewidth=2)
    ...
    for i in range(n_models-1):
        for j in range(i+1, n_models):
            ...
            lw = lw_undistorted  / dists_2d[i+1, j+1] * rdm_dists[i+1, j+1]
            plt.plot(locs2d[[i+1, j+1], 0], locs2d[[i+1, j+1], 1], color=col, linewidth=lw, solid_capstyle='round')
