...
This file contains the methods to batch process allen data
"""
...
def generate_design_random(nStimuli, repeats=3, duration=5, pause=0,
                           endzeros=20):
    # generates a design matrix for a given number of stimuli presented
    # in random order each stimulus is shown before any repetitions happen
    # duration and pause are measured in measurement cycles
    design = np.zeros((nStimuli * repeats
                       * (duration + pause) + endzeros, nStimuli + 1))
    design[:, -1] = 1
    ...
    for iRep in range(repeats):
        ...
        for iStim in range(nStimuli):
            design[(length * iStim + lengthAll * iRep):(
                length * (iStim + 1) + lengthAll * iRep),
                stimOrder[iStim]] = 1
    return design
...
def generate_timecourse(design, U0, sigma_noise=1, hrf=None,
    ...
    # generates a timecourse with random normal noise added after
    # the convolution with the hrf
    if ar_coeff is None:
        ar_coeff = .5
    ...
    for iNoise in range(1, noise.shape[0]):
        noise[iNoise] = (ar_coeff * noise[iNoise - 1]
                         + np.sqrt((1 - ar_coeff**2)) * noise[iNoise])
...
def get_random_sample(U, sigmaP, sigmaNoise=None, N=10):
    if sigmaNoise == 0 or sigmaNoise is None:
        ...
        if len(U.shape) == 1:
            ...
            rand2 = np.random.randn(N, U.shape[0], U.shape[1], U.shape[2])
        Usamp = (U
                 + np.matmul(rand1, np.linalg.cholesky(sigmaP).transpose())
                 + np.matmul(rand2,
                             np.linalg.cholesky(sigmaNoise).transpose()))
    return Usamp
